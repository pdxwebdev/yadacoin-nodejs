...
class Wallet {
    constructor(hostname, port, user, pass) {
        ...
        this._request('get_balance'); // This line is necessary in order to do the initial handshake between this wrapper and yadacoin-wallet-rpc; without it, the first request to the wrapper fails (subsequent request succeed, though.)
    }
}
...
Wallet.prototype.create_wallet = function(filename, password, language) {
    ...
    let params = {
        filename: filename || 'yadacoin_wallet',
        'password': password || '',
...
// transfer YadaCoin to a single recipient, or a group of recipients
...
// helper function to convert YadaCoin amount to atomic units
